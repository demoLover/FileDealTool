...
@property (nonatomic, strong) NSHashTable <GRPanoramaView *>*panoViews;
...
- (void)registerView:(GRPanoramaView *)view {
    [self.panoViews addObject:view];
}

- (void)unRegisterView:(GRPanoramaView *)view {
...
- (void)display:(CADisplayLink *)displayLink {
    ...
    for (GRPanoramaView *view in self.panoViews) {
        if (view) {
            [view display];
        }
    }
